...
answer = yes_no('Do you have an active session of putty running \n \
...
if numType == 'HIC':
    myFile = Path("F:/backesj/tempHic.txt")
    while myFile.is_file() == True:
        time.sleep(3)
    else:
        f = open('F:/backesj/tempHic.txt'.format(directory),'w')
        for i in beneList:
            f.write('{0}\n'.format(i))
        f.close()
else:
    myFile = Path("F:/backesj/tempSsn.txt")
    while myFile.is_file() == True:
        time.sleep(3)
    else:
        f = open('F:/backesj/tempSsn.txt'.format(directory),'w')
        f.write('{0}'.format(beneTuple))
        f.close()
def startPutty():

    sas = subprocess.Popen('C:\Program Files\SASHome\SASFoundation\9.3\Zone5 SASSSH.bat', shell=True)
    time.sleep(3)
startPutty()   
...
#    time.sleep(0.1)
#    while w.GetWindowText (w.GetForegroundWindow()) != 'SAS':
#        shell.AppActivate('SAS')
#        time.sleep(1)
#    if w.GetWindowText (w.GetForegroundWindow()) == 'SAS':
#        print('Switched Corretly')
#    else:
#        print('did the sas else')
#        time.sleep(1)
#        shell.AppActivate(inPID)
#        shell.AppActivate('SAS')
...
#        win32gui.ShowWindow(hwnd, win32con.SW_MAXIMIZE)
    ...
    pya.press('enter')
    time.sleep(1)
    pya.typewrite("%include 'F:/backesj.pullNums.sas';") 
    time.sleep(1)
    pya.press('f8')
    time.sleep(1)
#    pya.press('enter')  
...
myFile = Path("F:/backesj/tempHic.sas7bdat")
while myFile.is_file() != True:
    time.sleep(3)
else:
    print("\n Closing SAS")
...
if answer == True:
    print('\n Running sas to create HIC database )
    sasRoute()
