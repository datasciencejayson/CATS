...
#        print(file)
    ...
    def validateEntries():
        ...
        # to the user.
            ...
            if text.lower() != 'hic' and myFile.is_file():
...
def dbSetup():
    ...
    return beneDatabase, facDatabase, deadDatabase
...
def dbPulls():   
    ...
    if numType == 'HIC': 
        ...
        return dfDeadBene, dfBene, deadBenes
    else:
...
for i in range(1,4):
    beneDict2[i] = beneDict[i]

beneDict2[1] = beneDict[1]
...
writer = pd.ExcelWriter('%s:/Bene Phone List.xlsx' % file[0])
...
writerFormat('H:/', 'Bene Phone List')
...
beneTuple = createBeneTuple()

beneDatabase, facDatabase, deadDatabase = dbSetup()

dfDeadBene, dfBene, deadBenes = dbPulls()

beneDict = beneDictSetUp()

fullDict = truePeopleSearch()
